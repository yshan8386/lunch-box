        ...
        <div className="flex flex-col items-center p-8">
            <div className="w-72 bg-white border border-dashed border-gray-400 rounded p-6 font-mono shadow-md">
                ...
                <div className="text-center border-b border-dashed border-gray-400 pb-4 mb-4">
                    ...
                    <p className="text-gray-600 font-bold mt-1">{item.category_nm}</p>
                ...
                <div className="space-y-2 text-sm border-b border-dashed border-gray-400 pb-4 mb-4">
                    {!menus &&
                        <div className="flex justify-between">데이터가 없습니다.</div>
                    }
                  
                    {menus?.map((menu, index)=>(
                        <div key={menu.id}>
                            {index > 0 && <hr className="border-dashed border-gray-300 my-2" />}
                            <div className="flex justify-between">
                                <span className="text-gray-500">사람</span>
                                <span>{menu.person_nm}</span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span>₩{Number(menu.price).toLocaleString()}</span>
                            </div>
                            <div className="text-center">
                                <p className="text-xs text-gray-400 mb-1">RATING</p>
                                <p className="text-2xl tracking-widest">
                                    {'★'.repeat(menu.grade)}{'☆'.repeat(5 - menu.grade)}
                                </p>
                            </div>
                        </div>
                    ))}
            ...
            <div className="flex justify-center gap-2 mt-4 w-72">
                <button
                    ...
                    className="px-4 py-1.5 bg-white border border-gray-300 text-gray-500 text-sm font-semibold rounded-full hover:bg-gray-100 transition-colors"
                >
                목록으로
                </button>
                <button
                    onClick={() => navigate(`/lunch/write/${id}`)}
